...
const DEFAULT_EXEC_RUNTIME_ID: &str = "FAE_DEFAULT_EXECUTOR";
const DEFAULT_EXEC_RUNTIME_EVENT_CHANNEL_COUNT: usize = 1024;

#[derive(Debug)]
pub struct ExecRuntime {
    events: Channel<EnvEvent>,
    parent: Option<Env>,
    executors: HashMap<String, Arc<dyn Executor + Send + 'static>>,
}

impl ExecRuntime {
    pub fn new() -> Self {
        let events = Channel::with_cap(DEFAULT_EXEC_RUNTIME_EVENT_CHANNEL_COUNT);
        Self {
            events,
            parent: None,
