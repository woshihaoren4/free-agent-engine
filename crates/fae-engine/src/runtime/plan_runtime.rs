...
impl PlanRuntime {
    ...
    fn parse_plan_sub_id(id: &str) -> Option<(String,String)> {
        if id.starts_with(DEFAULT_PLAN_RUNTIME_PLAN_ID_PREFIX) {
            let ss = id.trim_start_matches(DEFAULT_PLAN_RUNTIME_PLAN_ID_PREFIX).split(DEFAULT_PLAN_RUNTIME_PLAN_ID_SPILT).collect::<Vec<_>>();
            if ss.len() != 2 {
                return None;
            }
    ...
    async fn get_plan_raw(ptr:&Arc<RwLock<HashMap<String, Arc<Mutex<PlanCtl>>>>>,id:&str) -> Option<Arc<Mutex<PlanCtl>>> {
    ...
    async fn remove_plan_raw(ptr:&Arc<RwLock<HashMap<String, Arc<Mutex<PlanCtl>>>>>, id: &str) -> Option<Arc<Mutex<PlanCtl>>> {
    ...
    fn task_result_callback(&self, mut result:TaskResult) {
        ...
        tokio::spawn(async move {
            ...
            let plan = if let Some(p) = plan {
                ...
                wd_log::log_error_ln!("[PlanRuntime:task_result_callback] plan not found: {:?}", result.agent_id);
                return;
            };
            let mut plan_lock = plan.lock().await;
    ...
    async fn query(&self, select: ThingSelect) -> anyhow::Result<Vec<Thing>> {
        return if let ThingSelect::Plan(pid, aid) = select {
            ...
            if let Some(p) = self.get_plan(&id).await {
                ...
                return Ok(vec![Thing::new(self.id().to_string()).add_item(ThingItem::Plan(info)).into_self()]);
            }
            Ok(Vec::new())
