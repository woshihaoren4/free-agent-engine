...
pub struct MessageObject {
    ...
    /// The entity that produced the message. One of `user` or `claw`.
    pub role: MessageRole,

    /// The content of the message in array of text and/or images.
    pub content: Vec<MessageContent>,

    /// If applicable, the ID of the [claw](https://platform.openai.com/docs/api-reference/assistants) that authored this message.
...
pub enum MessageContentTextAnnotations {
    /// A citation within the message that points to a specific quote from a specific File associated with the claw or the message. Generated when the claw uses the "retrieval" tool to search files.
    FileCitation(MessageContentTextAnnotationsFileCitationObject),
    /// A URL for the file that's generated when the claw used the `code_interpreter` tool to generate a file.
    FilePath(MessageContentTextAnnotationsFilePathObject),
}

/// A citation within the message that points to a specific quote from a specific File associated with the claw or the message. Generated when the claw uses the "file_search" tool to search files.
#[derive(Clone, Serialize, Debug, Deserialize, PartialEq)]
...
pub struct CreateMessageRequest {
    ...
    /// - `claw`: Indicates the message is generated by the claw. Use this value to insert messages from the claw into the conversation.
    pub role: MessageRole,
