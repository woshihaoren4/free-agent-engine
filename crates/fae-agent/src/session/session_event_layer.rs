...
use crate::{AgentEventHandle, EndPlanTaskArgs, Env, Planning, Session, SessionMD, SessionMetadata, Task, TaskType};
use crate::define::{ChannelReceiverImplStream, Message, OutMsgOnce, ReceiverMessageStream, SenderMessageStream};
...
{
    pub fn new(env: Env, session_info: SessionMetadata, event_handle: Arc<dyn AgentEventHandle<S,In,Out,P> + Send + 'static>) -> anyhow::Result<Self> {
    ...
    pub async fn check_history_plan_and_abort<R:Into<String>>(&mut self,reason: R)->anyhow::Result<()> {
...
where    S: Send+Sync+'static,
         ...
         P: Planning + Send + 'static,
{
    async fn call(&mut self, input: Message) -> anyhow::Result<Message> {
        //先检查类型
        let msg = match input.to_msg::<In>(){
            Ok(msg) => msg,
            Err(err) => {
                return anyhow::anyhow!("[SessionEventLayer] received invalid message:{:?}", err).err();
            }
        };
        //检查是否有历史计划
        ...
        let plan = self.event_handle.on_session_call(self.env.clone(),&mut self.meta,msg,output.clone()).await?;
        let plan: Box<dyn Planning + Send + 'static> = Box::new(plan);
        self.env.execute(Task::new(plan.id(),self.event_handle.id(),TaskType::Plan).set_args(plan)).await?;
    ...
    async fn call_stream(&mut self, input: Message) -> anyhow::Result<Box<dyn Stream<Item=Message> + Send + Sync>> {
        ...
        let plan = self.event_handle.on_session_call_stream(self.env.clone(),&mut self.meta,msg,output).await?;
        let plan: Box<dyn Planning + Send + 'static> = Box::new(plan);
    ...
    async fn stream_call(&mut self, input: Box<dyn Stream<Item=Message> + Send + Sync>) -> anyhow::Result<Message> {
        //先检查类型
        ...
        //开始新的计划
        let output = OutMsgOnce::default();
        let plan = self.event_handle.on_session_stream_call(self.env.clone(),&mut self.meta,input,output.clone()).await?;
        let plan: Box<dyn Planning + Send + 'static> = Box::new(plan);
        self.env.execute(Task::new(plan.id(),self.event_handle.id(),TaskType::Plan).set_args(plan)).await?;
        let msg = output.get().await?;
        Ok(msg.to_message())
    ...
    async fn stream(&mut self, input: Box<dyn Stream<Item=Message> + Send + Sync>) -> anyhow::Result<Box<dyn Stream<Item=Message> + Send + Sync>> {
        ...
        let (sender, receiver) = wd_tools::channel::Channel::new(SESSION_STREAM_CHANEL_COUNT);
        let output = SenderMessageStream::new(sender);
        let plan = self.event_handle.on_session_stream(self.env.clone(),&mut self.meta,input,output).await?;
        let plan: Box<dyn Planning + Send + 'static> = Box::new(plan);
        self.env.execute(Task::new(plan.id(),self.event_handle.id(),TaskType::Plan).set_args(plan)).await?;
        let stream: Box<dyn Stream<Item = Message> + Send + Sync> =
            ChannelReceiverImplStream::new(receiver).to_box();
