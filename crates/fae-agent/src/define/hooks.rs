...
impl Hook{
    pub fn agent_call_session_over<F>(agent_id:&str,session_id:&str,handle:impl FnOnce(&mut wd_event::Context,Box<dyn Any + Sync + Send + 'static>)->F + Send + Sync + 'static)
    where   
...
impl Trigger {
    pub fn agent_call_session_over<M: Message + Sync + Send + 'static>(agent_id:&str,session_id:&str,output:SenderMessageStream<M>){
        let key = format!("agent_call_session_over_{}:{}",agent_id,session_id);
